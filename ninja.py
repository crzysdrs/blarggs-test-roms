...
def multipart_roms(d, s, defines, includes):
    ...
    sub_o = n.build("libs/{}.o".format(name), rule="compile", inputs=[s], variables=compile_vars)    
...
for name, l in multis.items():
    ...
    objs = n.build("{}.multi.o".format(name), rule="compile", inputs="multi/cpu_instrs.s", variables=
            get_compile_vars(
                defines, includes
            )
    )
    linkfile = n.build("{}.link".format(name), rule="makelink", inputs=objs)
    link_vars={"linkfile": linkfile[0]}
    roms.extend(n.build("roms/{}/{}.gb".format(name, name), rule="link", inputs=linkfile + objs, variables=link_vars))
